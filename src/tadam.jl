...
const TADAM_θ1 = DefaultParameter(nlp -> eltype(nlp.meta.x0)(0.1), "T(0.1)")
 ...
 const TADAM_θ2 = DefaultParameter(nlp -> eltype(nlp.meta.x0)(1.0), "T(1.0)")
...
mutable struct TADAMSolver{T, V, M <: AbstractNLPModel{T, V}} <: AbstractOptimizationSolver
    ...
    d::V
    v::V
    v_hat::V     # Buffer for bias-corrected variance
    s::V
    sc::V        # Cauchy step
...
function TADAMSolver(
    ...
    v_hat = similar(nlp.meta.x0)
    s = similar(nlp.meta.x0)
    sc = similar(nlp.meta.x0)
    s_infty = similar(nlp.meta.x0)
    p = similar(nlp.meta.x0)
    return TADAMSolver{T, V, typeof(nlp)}(x, xt, gx, m, d, v, v_hat, s, sc, s_infty, p, zero(T), false, params)
end
...
function SolverCore.solve!(
    ...
    @. d = -gx
    # Initialize v properly so v_hat computation doesn't explode the step size
    @. v = gx^2 * (one(T) - β2) 
    ...
    while !done
        # 1. Compute bias-corrected variance for the subproblems
        @. v_hat = v / (oneT - β2^siter)

        # 2. Compute both bounding steps using v_hat
        solve_tadam_subproblem!(solver.s_infty, d, v_hat, solver.Δ, ϵ_v)
        compute_cauchy!(solver.sc, d, v_hat, solver.Δ, ϵ_v)
        
        # find_tau purely evaluates the Cauchy decrease condition
        tau = find_tau(solver.sc, solver.s_infty, d, v_hat, ϵ_v)
        ...
        if tau >= zero(T)
            ...
            step_underflow = x == xt
            # Evaluate predicted decrease using v_hat
            ΔTk = dot(d, s) - T(0.5) * dot(s .^ 2, sqrt.(v_hat) .+ ϵ_v)
            ...
            # Cauchy point numerical failure
            ρk = -one(T)
            step_underflow = false
        end

        # 3. Evaluate Trust Region Ratio
        if ρk >= η2
            solver.Δ = min(Δmax, γ2 * solver.Δ)
        elseif ρk < η1
            solver.Δ = solver.Δ * γ1
            β1max *= γ3  # Shrink momentum contribution on rejection
        end
        step_accepted = ρk >= η1
        solver.step_accepted = step_accepted 
        
        # 4. Apply RAW EMA Updates ONLY on accepted steps
        if step_accepted
            siter += 1
            x .= xt
...
function solve_tadam_subproblem!(s::V, d::V, v_hat::V, Δk::T, ϵ_v::T) where {V, T}
    @. s = min(Δk, max(-Δk, d / (sqrt(v_hat) + ϵ_v)))
end
...
function find_beta(p::V, mdotgx::T, norm_gx::T, β1::T, θ1::T, θ2::T, siter::Int) where {T, V}
    ...
    # ENFORCE THE [0, β1] BOUND FROM THE PAPER
    return max(zero(T), min(β1, min(β11, β12)))
end

function compute_cauchy!(sc::V, d::V, v_hat::V, Δk::T, ϵ_v::T) where {V, T}
    d_norm_sq = dot(d, d)
    d_H_d = dot(d .^ 2, sqrt.(v_hat) .+ ϵ_v)
    
    α_unc = d_norm_sq / (d_H_d + eps(T)) 
    α_max = Δk / (maximum(abs.(d)) + eps(T))
    
    α_C = min(α_unc, α_max)
    @. sc = α_C * d
end

function find_tau(sc::V, s_infty::V, d::V, v_hat::V, ϵ_v::T) where {V, T}
    # For TADAM: H_k^A is diagonal PD, so s_infty is the exact subproblem
    # solution and always dominates the Cauchy decrease. τ=1 is always valid.
    # We only fall back to τ∈[0,1) if s_infty has numerical issues.

    decrease_infty = dot(d, s_infty) - T(0.5) * dot(s_infty .^ 2, sqrt.(v_hat) .+ ϵ_v)
    decrease_sc    = dot(d, sc)      - T(0.5) * dot(sc      .^ 2, sqrt.(v_hat) .+ ϵ_v)

    # s_infty provides at least as much decrease as sc (it's the exact solution)
    decrease_infty ≥ decrease_sc - eps(T) && return one(T)

    # Numerical fallback: bisect for largest τ satisfying decrease ≥ decrease_sc
    decrease_sc ≤ zero(T) && return -one(T)  # Cauchy point itself is broken
    
    τ_lo, τ_hi = zero(T), one(T)
    for _ in 1:50
        τ_mid = (τ_lo + τ_hi) / 2
        s_mid = @. (one(T) - τ_mid) * sc + τ_mid * s_infty
        decrease_mid = dot(d, s_mid) - T(0.5) * dot(s_mid .^ 2, sqrt.(v_hat) .+ ϵ_v)
        decrease_mid ≥ decrease_sc ? (τ_lo = τ_mid) : (τ_hi = τ_mid)
        τ_hi - τ_lo < eps(T) && break
    end
    return τ_lo
end